describe 'Archive::Fixity' do
  ...
  it '.valid_checksum_types' do
    expect(Archive::Fixity.valid_checksum_types.map{|type| type.id}).to eq(Archive::Fixity.valid_checksum_ids)
  end

  it '.validate_checksum_types' do
  ...
  it '.get_digesters' do
    ...
    expect(digesters[:sha256].hexdigest("hello")).to eq(
  ...
  it '.fixity_from_checksum_values' do
    file_id = "dummy"
    checksum_values = [
