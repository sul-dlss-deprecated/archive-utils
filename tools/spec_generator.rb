...
class SpecGenerator
  ...
  def process_constructor(cls, mhash)
    ...
    if method.respond_to?(:docstring)
      ...
      output "expect(#{cls_instance}).to be_instance_of(#{cls.name})"
      @constructor_params.each do |p|
        unless p.name == 'opts'
          output "expect(#{cls_instance}.#{p.name}).to eq(#{p.name})"
        end
      end
      ...
      if arrays_and_hashes.size > 0
        ...
        arrays_and_hashes.each do |attr_name, type|
          output "expect(#{cls_instance}.#{attr_name}).to be_kind_of(#{type})"
        end
      end
      ...
      @constructor_params.each do |p|
        if p.name == 'opts'
          ...
          output "#{cls_instance} = #{cls.name}.new(#{param_list})"
          attribute_name_type_pairs(mhash).each do |name, type|
            output "expect(#{cls_instance}.#{name}).to eq(opts[:#{name}])"
          end
        end
      end
    ...
    attributes.values.each do |attribute|
      ...
      case scope
        when :class
          ...
          output "expect(#{read.path}).to eq(value)"
        when :instance
          output "@#{cls.name.snake_case}.#{write.name} value"
          output "expect(@#{cls.name.snake_case}.#{read.name}).to eq(value)"
      end
    ...
    methods.each do |method|
      begin
        ...
        if return_type == 'void'
          ...
          case scope
            when :class
              output "expect(#{method.path}(#{param_list})).to eq(0)"
            when :instance
              output "expect(@#{cls.name.snake_case}.#{method.name}(#{param_list})).to eq(0)"
          end
        end
